...
int PatchFinder::CalcSearchLevelAndWarpMatrix(MapPoint &p,
  ...
  TooN::Vector<3> v3Cam = se3CFromW * p.v3WorldPos;
  double dOneOverCameraZ = 1.0 / v3Cam[2];
  // Project the source keyframe's one-pixel-right and one-pixel-down vectors into the current view
  TooN::Vector<3> v3MotionRight = se3CFromW.get_rotation() * p.v3PixelRight_W;
  TooN::Vector<3> v3MotionDown = se3CFromW.get_rotation() * p.v3PixelDown_W;
  // Calculate in-image derivatives of source image pixel motions:
  mm2WarpInverse.T()[0] = m2CamDerivs *
      (v3MotionRight.slice<0,2>() - v3Cam.slice<0,2>() * v3MotionRight[2] * dOneOverCameraZ) * dOneOverCameraZ;
  mm2WarpInverse.T()[1] = m2CamDerivs *
      (v3MotionDown.slice<0,2>() - v3Cam.slice<0,2>() * v3MotionDown[2] * dOneOverCameraZ) * dOneOverCameraZ;
...
void PatchFinder::MakeTemplateCoarseCont(MapPoint &p) {
  ...
  TooN::Matrix<2> m2 = M2Inverse(mm2WarpInverse) * LevelScale(mnSearchLevel);
  ...
  for (int i = 0; !bNeedToRefreshTemplate && i < 2; i++) {
    TooN::Vector<2> v2Diff = m2.T()[i] - mm2LastWarpMatrix.T()[i];
...
void PatchFinder::MakeTemplateCoarseNoWarp(KeyFrame &k, int nLevel,
  ...
  CVD::Image<byte> &im = k.aLevels[nLevel].im;
...
bool PatchFinder::FindPatchCoarse(CVD::ImageRef irPos, KeyFrame &kf,
                                  unsigned int nRange) {
  mbFound = false;
  ...
  std::vector<ImageRef>::iterator i;
  std::vector<ImageRef>::iterator i_end;
...
void PatchFinder::MakeSubPixTemplate() {
  ...
  for (ir.x = 1; ir.x < mnPatchSize - 1; ir.x++)
    for (ir.y = 1; ir.y < mnPatchSize - 1; ir.y++) {
      ...
      TooN::Vector<3> v3Grad = unproject(v2Grad); // This adds the mean-difference jacobian..
      m3H += v3Grad.as_col() * v3Grad.as_row(); // Populate JTJ.
    }

  // Invert JTJ..
  TooN::Cholesky<3> chol(m3H);
...
double PatchFinder::IterateSubPix(KeyFrame &kf) {
  ...
  TooN::Vector<2> v2Center = LevelNPos(mv2SubPixPos, mnSearchLevel);
  CVD::BasicImage<byte> &im = kf.aLevels[mnSearchLevel].im;
  ...
  TooN::Vector<2> v2Base = v2Center - vec(mirCenter);

  // I.C. JT*d accumulator
  TooN::Vector<3> v3Accum = TooN::Zeros;
  ...
  unsigned long nRowOffset = &kf.aLevels[mnSearchLevel].im[CVD::ImageRef(0,1)] -
      &kf.aLevels[mnSearchLevel].im[CVD::ImageRef(0,0)];
  for (ir.y = 1; ir.y < mnPatchSize - 1; ir.y++) {
    pTopLeftPixel = &im[::ir(v2Base) + CVD::ImageRef(1,ir.y)]; // n.b. the x=1 offset, as with y
    for (ir.x = 1; ir.x < mnPatchSize - 1; ir.x++) {
  ...
  TooN::Vector<3> v3Update = mm3HInv * v3Accum;
