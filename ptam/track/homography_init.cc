...
namespace ptam {
bool HomographyInit::IsHomographyInlier(TooN::Matrix<3> m3Homography,
                                        HomographyMatch match) {
  TooN::Vector<2> v2Projected = project(m3Homography * unproject(match.v2CamPlaneFirst));
  TooN::Vector<2> v2Error = match.v2CamPlaneSecond - v2Projected;
  TooN::Vector<2> v2PixelError = match.m2PixelProjectionJac * v2Error;
  double dSquaredError = v2PixelError * v2PixelError;
  return (dSquaredError < mdMaxPixelErrorSquared);
}

double HomographyInit::MLESACScore(TooN::Matrix<3> m3Homography,
                                   HomographyMatch match) {
  TooN::Vector<2> v2Projected = project(m3Homography * unproject(match.v2CamPlaneFirst));
  TooN::Vector<2> v2Error = match.v2CamPlaneSecond - v2Projected;
  TooN::Vector<2> v2PixelError = match.m2PixelProjectionJac * v2Error;
...
void HomographyInit::RefineHomographyWithInliers() {
  ...
  vector<TooN::Matrix<2, 9> > vmJacobians;
  vector<TooN::Vector<2> > vvErrors;
  vector<double> vdErrorSquared;

  for (unsigned int i = 0; i < mvHomographyInliers.size(); i++) {
    // First, find error.
    TooN::Vector<2> v2First = mvHomographyInliers[i].v2CamPlaneFirst;
    TooN::Vector<3> v3Second = mm3BestHomography * unproject(v2First);
    TooN::Vector<2> v2Second = project(v3Second);
    TooN::Vector<2> v2Second_real = mvHomographyInliers[i].v2CamPlaneSecond;
    TooN::Vector<2> v2Error = mvHomographyInliers[i].m2PixelProjectionJac * (v2Second_real - v2Second);

    vdErrorSquared.push_back(v2Error* v2Error);
    vvErrors.push_back(v2Error);

    TooN::Matrix<2,9> m29Jacobian;
  ...
  wls.compute();
  TooN::Vector<9> v9Update = wls.get_mu();
  TooN::Matrix<3> m3Update;
...
void HomographyInit::BestHomographyFromMatches_MLESAC() {
  ...
  mm3BestHomography = TooN::Identity;
  ...
  for (int nR = 0; nR < 300 ; nR++) {
    ...
    TooN::Matrix<3> m3Homography = HomographyFromMatches(vMinimalMatches);
...
void HomographyInit::DecomposeHomography() {
  ...
  TooN::SVD<3> svd(mm3BestHomography);
  TooN::Vector<3> v3Diag = svd.get_diagonal();
  ...
  if (nCase != 1) {
    printf("  Homographyinit: This motion case is not implemented or is degenerate. Try again.\n");
    return;
  }
  ...
  for (int signs = 0; signs < 4; signs++) {
    // Eq 13
    decomposition.m3Rp = TooN::Identity;
  ...
  for (int signs = 0; signs < 4; signs++) {
    // Eq 15
    decomposition.m3Rp = -1 * TooN::Identity;
...
static double SampsonusError(TooN::Vector<2> &v2Dash,
                             const TooN::Matrix<3> &m3Essential,
                             TooN::Vector<2> &v2) {
  TooN::Vector<3> v3Dash = unproject(v2Dash);
  TooN::Vector<3> v3 = unproject(v2);

  double dError = v3Dash * m3Essential * v3;

  TooN::Vector<3> fv3 = m3Essential * v3;
  TooN::Vector<3> fTv3Dash = m3Essential.T() * v3Dash;

  TooN::Vector<2> fv3Slice = fv3.slice<0,2>();
  TooN::Vector<2> fTv3DashSlice = fTv3Dash.slice<0,2>();
...
void HomographyInit::ChooseBestDecomposition() {
  ...
  for (unsigned int i = 0; i < mvDecompositions.size(); i++) {
    ...
    for (unsigned int m = 0; m < mvHomographyInliers.size(); m++) {
      TooN::Vector<2> &v2 = mvHomographyInliers[m].v2CamPlaneFirst;
      double dVisibilityTest = (mm3BestHomography[2][0] * v2[0] +
          mm3BestHomography[2][1] * v2[1] + mm3BestHomography[2][2]) / decom.d;
  ...
  for (unsigned int i = 0; i < mvDecompositions.size(); i++) {
    ...
    int nPositive = 0;
    for (unsigned int m = 0; m < mvHomographyInliers.size(); m++) {
      TooN::Vector<3> v3 = unproject(mvHomographyInliers[m].v2CamPlaneFirst);
  ...
  if (dRatio < 0.9) {  // no ambiguity!
    ...
    for (int i = 0; i < 2; i++) {
      TooN::SE3<> se3 = mvDecompositions[i].se3SecondFromFirst;
      TooN::Matrix<3> m3Essential;
      for (int j = 0; j < 3; j++)
