...
public:
  ...
  // transformation.
  int CalcSearchLevelAndWarpMatrix(MapPoint &p, TooN::SE3<> se3CFromW,
  ...
  void MakeTemplateCoarse(MapPoint &p, TooN::SE3<> se3CFromW,
                          TooN::Matrix<2> &m2CamDerivs); // This also calculates the warp.
  ...
  inline TooN::Vector<2> GetCoarsePosAsVector() { return mv2CoarsePos; }
  ...
  inline TooN::Vector<2> GetSubPixPos()  { return mv2SubPixPos;   }  // Get result
  void SetSubPixPos(TooN::Vector<2> v2)  { mv2SubPixPos = v2;     }  // Set starting point

  // Get the uncertainty estimate of a found patch;
  // This for just returns an appropriately-scaled identity!
  TooN::Matrix<2> GetCov() {
...
protected:
  ...
  TooN::Vector<2> mv2CoarsePos;     // In the scale of level 0; hence the use of vector rather than ImageRef
