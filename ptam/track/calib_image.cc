...
bool CalibImage::MakeFromImage(Image<byte> &im) {
  ...
  // and run the corner finding on that.
  {
    ...
    glColor3f(1,0,1);
    glBegin(GL_POINTS);
    int nGate = GVars3::GV2.GetInt("CameraCalibrator.MeanGate",
                                   10, GVars3::SILENT);
    do
  ...
  if((int) mvCorners.size() <
     GVars3::GV2.GetInt("CameraCalibrator.MinCornersForGrabbedImage",
                        20, GVars3::SILENT))
    return false;
...
bool CalibImage::ExpandByAngle(int nSrc, int nDirn) {
  static GVars3::gvar3<int> gvnCornerPatchSize("CameraCalibrator.CornerPatchPixelSize", 20, GVars3::SILENT);
...
TooN::Matrix<2> CalibGridCorner::GetSteps(vector<CalibGridCorner> &vgc) {
...
void CalibImage::ExpandByStep(int n) {
  static GVars3::gvar3<double> gvdMaxStepDistFraction("CameraCalibrator.ExpandByStepMaxDistFrac", 0.3, GVars3::SILENT);
  static GVars3::gvar3<int> gvnCornerPatchSize("CameraCalibrator.CornerPatchPixelSize", 20, GVars3::SILENT);
...
void CalibImage::Draw3DGrid(ATANCamera &Camera, bool bDrawErrors) {
  ...
  if (bDrawErrors) {
    ...
    for (int i = 0; i < (int) mvGridCorners.size(); i++) {
      TooN::Vector<3> v3; v3[2] = 0.0;
      v3.slice<0, 2>() = vec(mvGridCorners[i].irGridPos);
      TooN::Vector<2> v2Pixels_Projected = Camera.Project(project(mse3CamFromWorld * v3));
...
vector<CalibImage::ErrorAndJacobians> CalibImage::Project(ATANCamera &Camera) {
  ...
  for (unsigned int n=0; n<mvGridCorners.size(); n++) {
    ...
    TooN::Vector<2> v2Image = Camera.Project(project(v3Cam));
    if (Camera.Invalid())
      continue;
