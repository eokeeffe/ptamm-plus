...
MapMaker::MapMaker(Map& m, const ATANCamera &cam) : mMap(m), mCamera(cam), bStop(false) {
...
void MapMaker::Reset() {
  ...
  mMap.keyframes.clear(); // TODO: actually erase old keyframes
...
void MapMaker::HandleBadPoints() {
  ...
  for (unsigned int i = 0; i < mMap.points.size(); i++)
    if (mMap.points[i]->bBad) {
      MapPoint *p = mMap.points[i];
      for (unsigned int j = 0; j < mMap.keyframes.size(); j++) {
        KeyFrame &k = *mMap.keyframes[j];
...
MapMaker::~MapMaker() {
  ...
  printf("Waiting for mapmaker to die..\n");
  mpThread->join();
  printf(" .. mapmaker has died.\n");
}

// Finds 3d coords of point in reference frame B from two z=1 plane projections
TooN::Vector<3> MapMaker::ReprojectPoint(TooN::SE3<> se3AfromB,
...
bool MapMaker::InitFromStereo(KeyFrame &kF,
  ...
  *pkSecond = kS;
  ...
  for (unsigned int i = 0; i < vMatches.size(); i++) {
    ...
    p->nSourceLevel = 0;
    p->v3Normal_NC = TooN::makeVector(0, 0, -1);
  ...
  printf("MapMaker: made initial map with %d points.\n", mMap.points.size());
  return true;
}
...
void MapMaker::ThinCandidates(KeyFrame &k, int nLevel) {
  ...
  for (meas_it it = k.mMeasurements.begin();
       it != k.mMeasurements.end(); it++) {
    if (!(it->second.nLevel == nLevel || it->second.nLevel == nLevel + 1))
      continue;
    irBusyLevelPos.push_back(CVD::ir_rounded(it->second.v2RootPos / LevelScale(nLevel)));
  }
...
void MapMaker::AddSomeMapPoints(int nLevel) {
  KeyFrame &kSrc = *(mMap.keyframes[mMap.keyframes.size() - 1]); // The new keyframe
...
void MapMaker::ApplyGlobalTransformationToMap(TooN::SE3<> se3NewFromOld) {
  for (unsigned int i = 0; i < mMap.keyframes.size(); i++)
    mMap.keyframes[i]->se3CfromW = mMap.keyframes[i]->se3CfromW * se3NewFromOld.inverse();

  TooN::SO3<> so3Rot = se3NewFromOld.get_rotation();
  for (unsigned int i = 0; i < mMap.points.size(); i++) {
    mMap.points[i]->v3WorldPos =
        se3NewFromOld * mMap.points[i]->v3WorldPos;
    mMap.points[i]->RefreshPixelVectors();
  }
}
...
void MapMaker::ApplyGlobalScaleToMap(double dScale) {
  for(unsigned int i = 0; i < mMap.keyframes.size(); i++)
    mMap.keyframes[i]->se3CfromW.get_translation() *= dScale;

  for (unsigned int i = 0; i < mMap.points.size(); i++) {
    (*mMap.points[i]).v3WorldPos *= dScale;
    (*mMap.points[i]).v3PixelRight_W *= dScale;
    (*mMap.points[i]).v3PixelDown_W *= dScale;
    (*mMap.points[i]).RefreshPixelVectors();
  }
}
...
bool MapMaker::AddPointEpipolar(KeyFrame &kSrc,
                                ...
                                int nLevel,
                                int nCandidate) {
  static CVD::Image<TooN::Vector<2> > imUnProj;
  ...
  std::vector<TooN::Vector<2> > &vv2Corners = kTarget.aLevels[nLevel].vImplaneCorners;
  ...
  if (!kTarget.aLevels[nLevel].bImplaneCornersCached) {
    for (unsigned int i = 0; i < vIR.size(); i++)  // over all corners in target img..
      vv2Corners.push_back(imUnProj[CVD::ir(LevelZeroPos(vIR[i], nLevel))]);
  ...
  pNew->v3Normal_NC = TooN::makeVector(0, 0, -1);
  pNew->irCenter = irLevelPos;
  pNew->v3Center_NC = unproject(mCamera.UnProject(v2RootPos));
  pNew->v3OneRightFromCenter_NC = unproject(mCamera.UnProject(v2RootPos + vec(CVD::ImageRef(nLevelScale,0))));
  pNew->v3OneDownFromCenter_NC  = unproject(mCamera.UnProject(v2RootPos + vec(CVD::ImageRef(0,nLevelScale))));
...
std::vector<KeyFrame*> MapMaker::NClosestKeyFrames(KeyFrame &k,
                                                   unsigned int N) {
  std::vector<pair<double, KeyFrame* > > vKFandScores;
  for (unsigned int i = 0; i < mMap.keyframes.size(); i++) {
    if (mMap.keyframes[i] == &k)
      continue;
    double dDist = KeyFrameLinearDist(k, *mMap.keyframes[i]);
    vKFandScores.push_back(make_pair(dDist, mMap.keyframes[i]));
  }
...
KeyFrame* MapMaker::ClosestKeyFrame(KeyFrame &k) {
  ...
  assert(nClosest != -1);
  return mMap.keyframes[nClosest];
}
...
bool MapMaker::NeedNewKeyFrame(KeyFrame &kCurrent) {
  ...
  if (dDist > GVars3::GV2.GetDouble("MapMaker.MaxKFDistWiggleMult", 1.0, GVars3::SILENT) * mdWiggleScaleDepthNormalized)
    return true;
  return false;
}
...
void MapMaker::BundleAdjustAll() {
  ...
  for (unsigned int i = 0; i < mMap.keyframes.size(); i++)
    if (mMap.keyframes[i]->bFixed)
      sFixed.insert(mMap.keyframes[i]);
    else
      sAdj.insert(mMap.keyframes[i]);
...
void MapMaker::BundleAdjustRecent() {
  if (mMap.keyframes.size() < 8) {  // Ignore this unless map is big enough
    mbBundleConverged_Recent = true;
    return;
  }
  ...
  KeyFrame *pkfNewest = mMap.keyframes.back();
  sAdjustSet.insert(pkfNewest);
  std::vector<KeyFrame*> vClosest = NClosestKeyFrames(*pkfNewest, 4);
  for (int i = 0; i < 4; i++)
  ...
  for (vector<KeyFrame*>::iterator it = mMap.keyframes.begin();
       it != mMap.keyframes.end(); it++) {
...
void MapMaker::BundleAdjust(std::set<KeyFrame*> sAdjustSet,
                            std::set<KeyFrame*> sFixedSet,
                            std::set<MapPoint*> sMapPoints, bool bRecent) {
  ...
  for (set<KeyFrame*>::iterator it = sFixedSet.begin();
       it != sFixedSet.end(); it++) {
  ...
  for (unsigned int i = 0; i < mMap.keyframes.size(); i++) {
    ...
    int nKF_BundleID = mView_BundleID[mMap.keyframes[i]];
    for (meas_it it = mMap.keyframes[i]->mMeasurements.begin();
        it!= mMap.keyframes[i]->mMeasurements.end();
        it++) {
  ...
  if (nAccepted < 0) {
    ...
    printf("!! MapMaker: Cholesky failure in bundle adjust. " \
           "   The map is probably corrupt: Ditching the map.\n");
    mbResetRequested = true;
    return;
  }
  ...
  if (nAccepted > 0) {
    ...
    for (std::map<KeyFrame*,int>::iterator itr = mView_BundleID.begin();
...
bool MapMaker::ReFind_Common(KeyFrame &k, MapPoint &p) {
  ...
  TooN::Matrix<2> m2CamDerivs = mCamera.GetProjectionDerivs();
  ...
  bool bFound = Finder.FindPatchCoarse(CVD::ir(v2Image), k, 4);  // Very tight search radius!
  if (!bFound) {
    p.pMMData->sNeverRetryKFs.insert(&k);
    return false;
  }
...
int MapMaker::ReFindInSingleKeyFrame(KeyFrame &k) {
  ...
  for (unsigned int i = 0; i < mMap.points.size(); i++)
    vToFind.push_back(mMap.points[i]);
...
void MapMaker::ReFindNewlyMade() {
  ...
  while (!mqNewQueue.empty() && mvpKeyFrameQueue.size() == 0) {
    ...
    for (unsigned int i = 0; i < mMap.keyframes.size(); i++)
      if (ReFind_Common(*mMap.keyframes[i], *pNew))
        nFound++;
  }
}
...
void MapMaker::ReFindFromFailureQueue() {
  ...
  std::vector<pair<KeyFrame*, MapPoint*> >::iterator it;
  int nFound = 0;
...
TooN::SE3<> MapMaker::CalcPlaneAligner() {
  unsigned int nPoints = mMap.points.size();
  if (nPoints < 10) {
    printf("  MapMaker: CalcPlane: too few points to calc plane.\n");
    return TooN::SE3<>();
  };

  int nRansacs = GVars3::GV2.GetInt("MapMaker.PlaneAlignerRansacs",
                                    100, GVars3::HIDDEN | GVars3::SILENT);
  ...
  for (int i = 0; i < nRansacs; i++) {
    ...
    TooN::Vector<3> v3Mean = 0.33333333 * (mMap.points[nA]->v3WorldPos +
                                     mMap.points[nB]->v3WorldPos +
                                     mMap.points[nC]->v3WorldPos);

    TooN::Vector<3> v3CA = mMap.points[nC]->v3WorldPos  -
        mMap.points[nA]->v3WorldPos;
    TooN::Vector<3> v3BA = mMap.points[nB]->v3WorldPos  -
        mMap.points[nA]->v3WorldPos;
    ...
    for (unsigned int i = 0; i < nPoints; i++) {
      TooN::Vector<3> v3Diff = mMap.points[i]->v3WorldPos - v3Mean;
  ...
  std::vector<TooN::Vector<3> > vv3Inliers;
  for (unsigned int i = 0; i < nPoints; i++) {
    TooN::Vector<3> v3Diff = mMap.points[i]->v3WorldPos - v3BestMean;
    double dDistSq = v3Diff * v3Diff;
    if (dDistSq == 0.0)
      continue;
    double dNormDist = fabs(v3Diff * v3BestNormal);
    if(dNormDist < 0.05)
      vv3Inliers.push_back(mMap.points[i]->v3WorldPos);
  }
...
void MapMaker::SaveMap() {
  printf("  MapMaker: Saving the map.... \n");
  ofstream ofs("map.dump");
  for (unsigned int i = 0; i < mMap.points.size(); i++) {
    ofs << mMap.points[i]->v3WorldPos << "  ";
    ofs << mMap.points[i]->nSourceLevel << endl;
  }
  ofs.close();
  ...
  for (unsigned int i = 0; i < mMap.keyframes.size(); i++) {
    ...
    ofs2 << mMap.keyframes[i]->se3CfromW << endl;
    ofs2.close();
  }
