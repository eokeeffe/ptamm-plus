...
#ifndef PTAM_CONSTRUCT_BUNDLE_H_
#define PTAM_CONSTRUCT_BUNDLE_H_
...
struct Camera {
  ...
  TooN::SE3<> se3CfW;
  TooN::SE3<> se3CfWNew;
  TooN::Matrix<6> m6U;  // Accumulator
  TooN::Vector<6> v6EpsilonA;  // Accumulator
  int nStartRow;
};
...
struct Point {
  inline Point() { nMeasurements = 0; nOutliers = 0;}
  TooN::Vector<3> v3Pos;
  TooN::Vector<3> v3PosNew;
  TooN::Matrix<3> m3V;  // Accumulator
  TooN::Vector<3> v3EpsilonB;  // Accumulator
  TooN::Matrix<3> m3VStarInv;
...
struct Meas {
  ...
  TooN::Vector<2> v2Found;
  TooN::Vector<2> v2Epsilon;
  TooN::Matrix<2,6> m26A;
  TooN::Matrix<2,3> m23B;
  TooN::Matrix<6,3> m63W;
  TooN::Matrix<6,3> m63Y;
...
public:
  ...
  int AddCamera(TooN::SE3<> se3CamFromWorld, bool bFixed); // Add a viewpoint. bFixed signifies that this one is not to be adjusted.
  int AddPoint(TooN::Vector<3> v3Pos);       // Add a map point.
  void AddMeas(int nCam, int nPoint, TooN::Vector<2> v2Pos, double dSigmaSquared); // Add a measurement
  ...
  TooN::Vector<3> GetPoint(int n);       // Point coords after adjustment
  TooN::SE3<> GetCamera(int n);            // Camera pose after adjustment
