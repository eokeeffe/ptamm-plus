...
inline void BundleTriangle_UpdateM6U_LL(TooN::Matrix<6> &m6U,
                                        TooN::Matrix<2,6> &m26A) {
  for (int r = 0; r < 6; r++)
    for (int c = 0; c <= r; c++)
      m6U(r,c) += m26A.T()(r,0)*m26A(0,c) + m26A.T()(r,1)*m26A(1,c);
}
inline void BundleTriangle_UpdateM3V_LL(TooN::Matrix<3> &m3V,
                                        TooN::Matrix<2,3> &m23B) {
  for (int r = 0; r < 3; r++)
    for (int c = 0; c <= r; c++)
...
Bundle::Bundle(const ATANCamera &TCam) : mCamera(TCam) {
  mnCamsToUpdate = 0;
  mnStartRow = 0;
  GVars3::GV3::Register(mgvnMaxIterations,
                        "Bundle.MaxIterations", 20,  GVars3::SILENT);
  GVars3::GV3::Register(mgvdUpdateConvergenceLimit,
                        "Bundle.UpdateSquaredConvergenceLimit", 1e-06, GVars3::SILENT);
  GVars3::GV3::Register(mgvnBundleCout, "Bundle.Cout", 0, GVars3::SILENT);
}

// Add a camera to the system, return value is the bundle adjuster's ID for the camera
int Bundle::AddCamera(TooN::SE3<> se3CamFromWorld, bool bFixed) {
...
int Bundle::AddPoint(TooN::Vector<3> v3Pos) {
  int n = mvPoints.size();
  Point p;
...
void Bundle::AddMeas(int nCam, int nPoint, TooN::Vector<2> v2Pos,
...
void Bundle::ClearAccumulators() {
  for (size_t i = 0; i < mvPoints.size(); ++i) {
    mvPoints[i].m3V = TooN::Zeros;
    mvPoints[i].v3EpsilonB = TooN::Zeros;
  }
  for (size_t i = 0; i < mvCameras.size(); ++i) {
    mvCameras[i].m6U = TooN::Zeros;
    mvCameras[i].v6EpsilonA = TooN::Zeros;
  }
}
...
int Bundle::Compute(bool *pbAbortSignal) {
  ...
  static GVars3::gvar3<string> gvsMEstimator("BundleMEstimator", "Tukey", GVars3::SILENT);
  ...
  while (!mbConverged  && !mbHitMaxIterations && !*pbAbortSignal) {
    ...
    if (*gvsMEstimator == "Cauchy") {
      ...
      printf("Invalid BundleMEstimator selected !! \n");
      printf("Defaulting to Tukey.\n");
      *gvsMEstimator = "Tukey";
      bNoError = Do_LM_Step<Tukey>(pbAbortSignal);
    };
...
inline void Bundle::ProjectAndFindSquaredError(Meas &meas) {
  ...
  meas.bBad = false;
  TooN::Vector<2> v2ImPlane = project(meas.v3Cam);
...
bool Bundle::Do_LM_Step(bool *pbAbortSignal) {
  ...
  for (std::list<Meas>::iterator itr = mMeasList.begin();
       itr != mMeasList.end(); itr++) {
    Meas &meas = *itr;
  ...
  static GVars3::gvar3<double> gvdMinSigma("Bundle.MinTukeySigma", 0.4, GVars3::SILENT);
  ...
  for (std::list<Meas>::iterator itr = mMeasList.begin();
    ...
    TooN::Matrix<2> m2CamDerivs = dWeight * meas.m2CamDerivs;

    const double dOneOverCameraZ = 1.0 / meas.v3Cam[2];
    const TooN::Vector<4> v4Cam = unproject(meas.v3Cam);

    // Calculate A: (the proj derivs WRT the camera)
    if (cam.bFixed) {
      meas.m26A = TooN::Zeros;
    } else {
      for (int m = 0; m < 6; m++) {
        const TooN::Vector<4> v4Motion = TooN::SE3<>::generator_field(m, v4Cam);
    ...
    for (int m = 0; m < 3; m++) {
      const TooN::Vector<3> v3Motion = cam.se3CfW.get_rotation().get_matrix().T()[m];
      TooN::Vector<2> v2CamFrameMotion;
  ...
  while (dNewError > dCurrentError && !mbConverged &&
         !mbHitMaxIterations && !*pbAbortSignal) {
    // Rest of part (ii) : find V*i inverse
    for (std::vector<Point>::iterator itr = mvPoints.begin();
         itr != mvPoints.end(); itr++) {
      Point &point = *itr;
      TooN::Matrix<3> m3VStar = point.m3V;
      if (m3VStar[0][0] * m3VStar[1][1] * m3VStar[2][2] == 0) {
        point.m3VStarInv = TooN::Zeros;
      } else {
    ...
    TooN::Matrix<> mS(mnCamsToUpdate * 6, mnCamsToUpdate * 6);
    mS = TooN::Zeros;
    TooN::Vector<> vE(mnCamsToUpdate * 6);
    vE = TooN::Zeros;

    TooN::Matrix<6> m6; // Temp working space
    TooN::Vector<6> v6; // Temp working space
    ...
    for (unsigned int i = 0; i < mvPoints.size(); i++) {
      ...
      Meas* pMeas_ij;
      Meas* pMeas_ik;
      TooN::Matrix<6,3> m63_MIJW_times_m3VStarInv;
    ...
    TooN::Vector<> vCamerasUpdate(mS.num_rows());
    vCamerasUpdate = TooN::Cholesky<>(mS).backsub(vE);

    // Part (iv): Compute the map updates
    TooN::Vector<> vMapUpdates(mvPoints.size() * 3);
    for (unsigned int i = 0; i < mvPoints.size(); i++) {
      TooN::Vector<3> v3Sum;
      v3Sum = TooN::Zeros;
      for (unsigned int j = 0; j < mvCameras.size(); j++) {
      ...
      TooN::Vector<3> v3 = mvPoints[i].v3EpsilonB - v3Sum;
    ...
    for (unsigned int j = 0; j < mvCameras.size(); j++) {
      if(mvCameras[j].bFixed)
        ...
        mvCameras[j].se3CfWNew =
            TooN::SE3<>::exp(vCamerasUpdate.slice(mvCameras[j].nStartRow, 6)) *
            mvCameras[j].se3CfW;
    }
    for(unsigned int i=0; i<mvPoints.size(); i++)
...
double Bundle::FindNewError() {
  ...
  for (std::list<Meas>::iterator itr = mMeasList.begin();
    ...
    Meas &meas = *itr;
    // Project the point.
    TooN::Vector<3> v3Cam = mvCameras[meas.c].se3CfWNew * mvPoints[meas.p].v3PosNew;
    if (v3Cam[2] <= 0) {
      dNewError += 1.0;
      printf(".");
      continue;
    ...
    TooN::Vector<2> v2Image   = mCamera.Project(v2ImPlane);
    TooN::Vector<2> v2Error =   meas.dSqrtInvNoise * (meas.v2Found - v2Image);
...
void Bundle::GenerateMeasLUTs() {
  ...
  for (std::list<Meas>::iterator it = mMeasList.begin();
       it != mMeasList.end(); it++)
...
void Bundle::GenerateOffDiagScripts() {
  for (unsigned int i = 0; i < mvPoints.size(); i++) {
    ...
    for (std::set<int>::iterator it_j = p.sCameras.begin();
      ...
      for (std::set<int>::iterator it_k = p.sCameras.begin();
           it_k != it_j; it_k++) {
        int k = *it_k;
...
TooN::Vector<3> Bundle::GetPoint(int n) {
  return mvPoints.at(n).v3Pos;
}

TooN::SE3<> Bundle::GetCamera(int n) {
