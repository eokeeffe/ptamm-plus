...
void ARDriver::Init() {
  mbInitialised = true;
  mirFBSize = GVars3::GV3::get<ImageRef>("ARDriver.FrameBufferSize",
                                 ImageRef(1200, 900), GVars3::SILENT);
  ...
  MakeFrameBuffer();
  mGame.Init();
  printf("Test here in Init ARDriver.\n");
}
...
void ARDriver::Render(Image<Rgb<byte> > &imFrame, TooN::SE3<> se3CfromW) {
  if (!mbInitialised) {
    Init();
    Reset();
  };
  ...
  if (bFirstRun) {
    ...
    for (int ystep = 0; ystep< nStepsY; ystep++) {
      ...
      for (int xstep = 0; xstep <= nStepsX; xstep++)
        for (int yystep = ystep; yystep<=ystep+1; yystep++) {
          // Two y-coords in one go - magic.
          TooN::Vector<2> v2Iter;
          ...
          TooN::Vector<2> v2UFBDistorted = v2Iter;
          TooN::Vector<2> v2UFBUnDistorted = mCamera.UFBLinearProject(mCamera.UFBUnProject(v2UFBDistorted));
