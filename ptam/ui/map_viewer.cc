...
void MapViewer::DrawMapDots() {
  ...
  for (size_t i = 0; i < mMap.points.size(); i++) {
    TooN::Vector<3> v3Pos = mMap.points[i]->v3WorldPos;
    glColor(get_level_color(mMap.points[i]->nSourceLevel));
...
void MapViewer::DrawMap(SE3<> se3CamFromWorld) {
  ...
  for (size_t i = 0; i < mMap.keyframes.size(); i++)
    DrawCamera(mMap.keyframes[i]->se3CfromW, true);
